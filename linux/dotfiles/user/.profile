_profile_xdg () {
    XDG_CACHE_HOME="${HOME}/.cache";       export XDG_CACHE_HOME;  /usr/bin/mkdir -p "${XDG_CACHE_HOME}"  # cache directory
    XDG_CONFIG_HOME="${HOME}/.config";     export XDG_CONFIG_HOME; /usr/bin/mkdir -p "${XDG_CONFIG_HOME}" # config directory
    XDG_DATA_HOME="${HOME}/.local/share";  export XDG_DATA_HOME;   /usr/bin/mkdir -p "${XDG_DATA_HOME}"   # data directory
    XDG_STATE_HOME="${HOME}/.local/state"; export XDG_STATE_HOME;  /usr/bin/mkdir -p "${XDG_STATE_HOME}"  # state directory
    XDG_APP_HOME="${HOME}/.apps";          export XDG_APP_HOME;    /usr/bin/mkdir -p "${XDG_APP_HOME}"    # apps gui directory
    XDG_BIN_HOME="${HOME}/.local/bin";     export XDG_BIN_HOME;    /usr/bin/mkdir -p "${XDG_BIN_HOME}"    # apps cli directory
}
...
_profile_wine () {
    WINEPREFIX="${HOME}/src/wine/default"; export WINEPREFIX; /usr/bin/mkdir -p "${WINEPREFIX}"
}
